...
适用版本：TRAE 国内版（api.trae.cn / solo），模型为豆包(Seed)、DeepSeek、GLM、Kimi、MiniMax、Qwen 等国内模型。
（注意：国际版 trae.ai 才有 GPT / Gemini，国内版无，请勿混入。）
...
数据源：
1. MODEL_TABLE：内置国内版模型清单（档位 / 倍率 / Lite是否可用 / 能力），依据国内版文档整理
...
# 倍率以 Seed-2.1-Turbo / Seed-Code ≈ 1.0 为基准，按国内版相对消耗估算
# （TRAE 国内版未公开逐模型积分倍率，以下为相对排序的合理估计，可经历史用量校正）。
# Lite 会员对 Seed-2.1-Turbo / Seed-Code 有 2.5 折优惠，故二者最省。
# tier: eco=经济, std=标准, flagship=旗舰
# lite: Lite 会员默认可用性（保守默认全 True；实际受限模型请用 model_overrides 标记 lite:false）
MODEL_TABLE = [
    # —— 经济档（倍率 ≤ 1.2，省积分；Lite 有 Seed 系列 2.5 折）——
    {"name": "Seed-Code",               "tier": "eco",       "multiplier": 1.0, "lite": True,  "caps": "编程,中文"},
    {"name": "Seed-2.1-Turbo",         "tier": "eco",       "multiplier": 1.0, "lite": True,  "caps": "推理,中文(2.5折)"},
    ...
    {"name": "Qwen3.8-Max",            "tier": "std",       "multiplier": 1.4, "lite": True,  "caps": "推理"},
    {"name": "Qwen3.7-Plus",           "tier": "std",       "multiplier": 1.2, "lite": True,  "caps": "推理"},
...
TIER_HINT = {
    "eco": "日常/简单任务：Seed-Code、Turbo、Flash 类，倍率低、省钱（Seed 系列 Lite 还享 2.5 折）",
    "std": "复杂编程/推理：DeepSeek-V4-Pro、Kimi-K2.7、GLM-5.2 类，性价比均衡",
    "flagship": "疑难/长上下文：最强模型，能力强但贵",
}

# 预算包络（今日 suggested）-> 允许档位
# off 日单独处理；非 off 日按包络大小放开档位
ECO_CEIL = 150      # 包络 ≤ 150 → 仅经济
STD_CEIL = 500      # 150 < 包络 ≤ 500 → 经济+标准；> 500 → 全开放（含旗舰，如有）
